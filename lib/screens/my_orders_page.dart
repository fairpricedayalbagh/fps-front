...
class MyOrder {
  final int id;
  final String
  status; // PENDING, PAID, CANCELLED, RECEIVED, READY, DELIVERED, etc.
  ...
  factory MyOrder.fromJson(Map<String, dynamic> j) {
    ...
    return MyOrder(
      ...
      totalAmount: toDouble(j['total_amount'] ?? j['amount']),
...
class MyOrderLine {
  ...
  factory MyOrderLine.fromJson(Map<String, dynamic> j) {
    ...
    return MyOrderLine(
      ...
      unitPrice: d(j['unit_price']),
      lineTotal: d(j['line_total']),
            ...
            Expanded(
              child: _results.isEmpty && !_loading
                  ? Center(
                      child: Text(
                        "No products found",
                        style: TextStyle(color: kTextPrimary.withOpacity(0.6)),
                      ),
                    )
                  : ListView.separated(
