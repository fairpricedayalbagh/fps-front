...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  final UpiMode _upiMode = UpiMode.qr;
  bool _loading = false;
    ...
    try {
      ...
      await showDialog(
        ...
        builder: (ctx) => AlertDialog(
          ...
          content: const Text(
            "Your order has been placed. Please wait for admin confirmation regarding price and availability.",
          ),
          actions: [
            TextButton(
              onPressed: () {
    ...
    try {
      ...
      if (res.statusCode != 200 && res.statusCode != 201) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Create order failed: ${msg ?? res.statusCode}'),
            ),
          );
        }
        return null;
